...
export function activate(context: ExtensionContext) {
    const executable = workspace.getConfiguration("mpfls").get<string>("executable");
    context.subscriptions.push(startLangServer(executable, ["-vv"], ["yaml"]));
    // For TCP server needs to be started seperately
    // context.subscriptions.push(startLangServerTCP(2087, ["yaml"]));
}
